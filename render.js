...
(function () {
  ...
  C.metrics.forEach(function(m, i) {
    ...
    if (match) {
      ...
      // store on the element for the observer below
  ...
  // count-up observer — fires when each metric-num scrolls into view
  ...
  document.querySelectorAll('.metric-num').forEach(function(numEl) {
    ...
    var obs = new IntersectionObserver(function(entries) {
      ...
      fired = true;
      obs.disconnect();
      var target = numEl._countTarget, dur = 1400, start = null;
      function step(ts) {
        if (!start) start = ts;
